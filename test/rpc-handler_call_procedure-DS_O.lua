...
describe("impl.rpc-handler", function()
  ...
  describe("WHEN given a request of type ProcedureType.DS_O", function()
    ...
    setup(function()
      ...
      request = {}
      request.type = qlua.RPC.ProcedureType.DS_O
    end)
    ...
    insulate("WITH arguments", function()
      ...
      setup(function()
        ...
        request_args = qlua.datasource.O.Request()
        ...
        datasource = {
          O = spy.new(function(candle_index) return proc_result end)
        }
      ...
      it("SHOULD call the 'O' function on the datasource returned by its 'get_datasource' function, passing self and the 'candle_index' argument to it", function()
          
        sut.call_procedure(request.type, request.args)
        
        assert.spy(datasource.O).was.called_with(datasource, request_args.candle_index)
      end)
    
      it("SHOULD return a qlua.datasource.O.Result instance", function()
          
        local actual_result = sut.call_procedure(request.type, request.args)
        local expected_result = qlua.datasource.O.Result()
