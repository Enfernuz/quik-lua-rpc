...
describe("impl.rpc-handler", function()
  ...
  describe("WHEN given a request of type ProcedureType.DS_SET_EMPTY_CALLBACK", function()
    ...
    setup(function()
      ...
      request = {}
      request.type = qlua.RPC.ProcedureType.DS_SET_EMPTY_CALLBACK
    end)
    ...
    insulate("WITH arguments", function()
      ...
      setup(function()
        ...
        request_args = qlua.datasource.SetEmptyCallback.Request()
        ...
        datasource = {
          SetEmptyCallback = spy.new(function(candle_index) return proc_result end)
        }
      ...
      it("SHOULD call the 'SetEmptyCallback' function on the datasource returned by its 'get_datasource' function, passing self to it", function()
          
        sut.call_procedure(request.type, request.args)
        
        assert.spy(datasource.SetEmptyCallback).was.called_with(datasource)
      end)
    
      it("SHOULD return a qlua.datasource.SetEmptyCallback.Result instance", function()
          
        local actual_result = sut.call_procedure(request.type, request.args)
        local expected_result = qlua.datasource.SetEmptyCallback.Result()
