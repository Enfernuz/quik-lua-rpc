...
describe("impl.rpc-handler", function()
  ...
  describe("WHEN given a request of type ProcedureType.GET_BUY_SELL_INFO_EX", function()
    ...
    setup(function()
      ...
      request = {}
      request.type = qlua.RPC.ProcedureType.GET_BUY_SELL_INFO_EX
    end)
    ...
    insulate("WITH arguments", function()
      ...
      setup(function()
        ...
        request_args = qlua.getBuySellInfoEx.Request()
        ...
        _G.getBuySellInfoEx = spy.new(function(firm_id, client_code, class_code, sec_code, price) return proc_result end)
      end)
      ...
      it("SHOULD call the global 'getBuySellInfoEx' function once, passing the procedure arguments to it AND the 'price' argument must be converted to number", function()
        
        local response = sut.call_procedure(request.type, request.args)
    
        assert.spy(_G.getBuySellInfoEx).was.called_with(request_args.firm_id, request_args.client_code, request_args.class_code, request_args.sec_code, tonumber(request_args.price))
      end)
    
      it("SHOULD return a qlua.getBuySellInfoEx.Result instance", function()
          
        local actual_result = sut.call_procedure(request.type, request.args)
        local expected_result = qlua.getBuySellInfoEx.Result()
      ...
      it("SHOULD return a protobuf object which string-serialized form equals to that of the expected result", function()
        ...
        struct_converter.getBuySellInfoEx.BuySellInfoEx(proc_result, expected_result.buy_sell_info_ex)
      ...
      insulate("AND the global 'getBuySellInfoEx' function returns nil", function()
          
        setup(function()
          _G.getBuySellInfoEx = spy.new(function(firm_id, client_code, class_code, sec_code, price) return nil end)
        end)
      
        it("SHOULD raise an error", function()
          
          local expected_error_msg = string.format("Процедура getBuySellInfoEx(%s, %s, %s, %s, %s) возвратила nil.", request_args.firm_id, request_args.client_code, request_args.class_code, request_args.sec_code, request_args.price)
