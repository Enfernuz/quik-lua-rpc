...
function QluaService:get_datasource(datasource_uuid) 
...
request_handlers[qlua_msg.ProcedureType.CREATE_DATA_SOURCE] = function(request_args) 
  ...
  if ds == nil then
    ...
    QluaService.datasources[result.datasource_uuid] = ds
    return result
  end
end
...
request_handlers[qlua_msg.ProcedureType.DS_SET_UPDATE_CALLBACK] = function(request_args) 
  ...
  local ds = QluaService:get_datasource(args.datasource_uuid)
...
local function process_rpc_request(request) 
  ...
  if rep_socket_addr ~= nil then
    ...
    self.poller:add(self.rep_socket, zmq.POLLIN, function() 
      ...
      if ok and ret ~= nil and ret ~= -1 then
        ...
        local response = process_rpc_request(request)
